...
新视频的首个媒体请求不会等待测速：有缓存时立即使用缓存赢家，没有缓存时先使用 B 站原始 CDN。脚本只会在
视频出现至少 15 秒后的后续媒体请求中测速；如果这段时间没有后续请求，就继续原线路而不强行测速。
...
- 换 Wi‑Fi、切换蜂窝网络后：工具会让旧选择失效，首个请求先走原始 CDN，安全延迟后再重新选择。
...
默认策略为：成功结果缓存 3 小时、失败结果缓存 10 分钟；新视频 15 秒后以及每 15 分钟只比较当前节点和
最佳备用节点，各读取 128 KiB。完整测速也使用每节点 128 KiB 并行请求，最长约 4 秒，只在无缓存、明显
变慢、节点失败、换网或候选耗尽时发生。iOS 网络工具看不到播放器缓冲值，因此无法像浏览器扩展一样每 2 秒
读取缓冲；这里的 15 秒安全延迟是请求层的保守近似。
