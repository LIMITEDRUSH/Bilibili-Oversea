...
2. `content.js` validates the bridge boundary, watches the video element, and performs bounded candidate probes in the Bilibili page context so the CDN receives the normal playback referrer.
3. `worker.js` keeps signed URLs in memory, validates and ranks probe results, and owns session rules.
